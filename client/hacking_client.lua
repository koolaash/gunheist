...
local scaleform = nil
local ClickReturn 
local lives = Config.lives --7 Lives in GTA Online
...
function StartHacking()
	...
	TriggerEvent(Config.hud .. ':displayHud', false)
		...
		if HasScaleformMovieLoaded(scaleform) then
			...
			if GetScaleformMovieFunctionReturnBool(ClickReturn) then -- old native?
				...
				elseif ProgramID == 83 then  --BRUTEFORCE.EXE
					...
					PushScaleformMovieFunctionParameterString(Config.gamePassword)
					PopScaleformMovieFunctionVoid()
			...
			if hacking then
				hacking = false
				TriggerEvent(Config.hud .. ':displayHud', true)
				SetScaleformMovieAsNoLongerNeeded(scaleform)
			end
